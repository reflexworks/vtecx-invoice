...
const paginateRecords = (records: PurchaseOrderRecord[]): PurchaseOrderRecord[][] => {
  if (records.length === 0) return [[]]
  const pages: PurchaseOrderRecord[][] = []
  let i = 0
...
/** 明細データ行 */
const renderRecordRows = (records: PurchaseOrderRecord[], startIndex: number) =>
  records.map((r, i) => {
...
const renderTotalTable = (data: PurchaseOrderEntry, y: number) => {
    ...
    >
      ...
      <tr>
        ...
        <td style={td('LRTB', 'right', ROW_H + 4, SUB_BG_COLOR)}>
          <div style={css_text_font(10, 1, false, false, true)}>{'ご注文金額（税込）'}</div>
        </td>
        <td style={td('LRTB', 'right', ROW_H + 4, SUB_BG_COLOR)}>
...
/** 備考欄（1ページ目固定Y座標） */
const renderFirstPageRemarks = (data: PurchaseOrderEntry) => {
  return (
    <>
  ...
  return (
    ...
    >
      ...
      <table style={absTable(LAYOUT.pdf_info.x, LAYOUT.pdf_info.y, LAYOUT.pdf_info.w)}>
        <tr>
          <td style={td('', 'right', 18)}>
            <div style={css_text_font(9)}>注文番号 : {data.purchase_order_code}</div>
          </td>
        </tr>
        <tr>
          <td style={td('', 'right', 18)}>
      ...
      <table style={absTable(LAYOUT.notice.x, LAYOUT.notice.y, LAYOUT.notice.w)}>
        <tr>
          <td style={td('', 'left', 18)}>
            <div style={css_text_font(9)}>下記の通り、注文申し上げます。</div>
          </td>
        </tr>
      </table>
        ...
        <tr>
          <td style={{ ...td('B', 'left', 22), valign: 'top' }}>
            <div style={{ ...css_text_font(14, 1, false, false, true), valign: undefined }}>
              ご注文金額：¥ {addFigure(String(data.total_amount ?? 0))}-
            </div>
          </td>
        </tr>
      </table>
      ...
      >
        ...
        <tr>
          <td style={td('LRTB', 'center', 22)}>
            <div style={css_text_font(8)}>納品場所</div>
          </td>
          <td style={{ ...td('LRTB', 'left', 22), padding: '0 10px' }}>
            <div style={css_text_font(9)}>{data.delivery_location ?? ''}</div>
          </td>
        </tr>
      </table>
...
export const getHtmlTemplate = async (data: PurchaseOrderEntry, file_name: string): Promise<string> => {
  console.log(data)
