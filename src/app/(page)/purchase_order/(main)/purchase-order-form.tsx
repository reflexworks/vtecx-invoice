...
type ErrorState = {
  ...
  subject?: string
  records?: { [key: number]: { description?: string } }
}
...
export default function PurchaseOrderForm({
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setData({
        ...initialData,
        purchase_order: {
          ...initialData.purchase_order,
          delivery_date: strToDate(initialData.purchase_order.delivery_date)
        },
  ...
  const handleChange = (field: string, value: string) => {
    setIsDirty(true)
    setData((prev) => ({ ...prev, purchase_order: { ...prev.purchase_order, [field]: value } }))
  }
  ...
  const handleRecordChange = (index: number, field: string, value: string | number) => {
    ...
    newRecords[index] = {
      ...newRecords[index],
      [field]: field === 'tax_rate' ? Number(value) : value
    }
    setIsDirty(true)
  ...
  const handleSave = async () => {
    ...
    if (!data.purchase_order.customer_name.trim())
      newErrors.customer_name = '宛名を入力してください'
    const recordErrors: { [key: number]: any } = {}
    data.record.forEach((item, index) => {
      if (!item.description.trim()) recordErrors[index] = { description: '品名を入力してください' }
    })
    ...
    const entry = {
      purchase_order: {
        ...data.purchase_order,
        issue_date: dateToNum(data.purchase_order.issue_date),
        delivery_date: dateToNum(data.purchase_order.delivery_date),
    ...
    if (result && 'error' in result) {
      ...
      setIsDirty(false)
      setSnackbar({ open: true, message: '注文書を正常に保存しました！', severity: 'success' })
      setTimeout(() => router.push(`/${uid}/purchase_order`), 1000)
    }
  }
          ...
          <Paper sx={{ p: 6, borderRadius: 2, minHeight: '100%' }}>
            ...
            <Grid container spacing={4}>
              ...
              <Grid item xs={12} md={7}>
                <Box sx={{ mb: 4 }}>
                  ...
                  >
                    <TextField
                      variant="standard"
                      value={data.purchase_order.customer_name}
                      onChange={(e) => {
                  ...
                  >
                    ...
                    <TextField
                      fullWidth
                      variant="standard"
                      value={data.purchase_order.subject}
                  ...
                  <Typography sx={{ mt: 6 }}>下記の通り、発注いたします。</Typography>
                  ...
                  <Box sx={{ mt: 2, border: '1px solid #ccc', width: '75%' }}>
                    {[
                      { label: '納期', field: 'delivery_date', isDate: true },
                      { label: '納品場所', field: 'delivery_location', isDate: false }
                    ].map((row) => (
                      <Box
                        key={row.field}
                        sx={{
                      ...
                      >
                        ...
                        <Box sx={{ flexGrow: 1, px: 1, py: 0.5 }}>
                          {row.isDate ? (
                            <DatePicker
                              format="yyyy/MM/dd"
                              value={data.purchase_order[row.field] as Date | null}
                              disabled={readOnly}
                              onChange={(newValue) => {
                                setData((prev) => ({
                                  ...prev,
                                  purchase_order: { ...prev.purchase_order, [row.field]: newValue }
                                }))
                              }}
                              slotProps={{
                                textField: {
                            ...
                            <TextField
                              fullWidth
                              variant="standard"
                              value={data.purchase_order[row.field] ?? ''}
              ...
              <Grid item xs={12} md={5}>
                ...
                >
                  {data.purchase_order.purchase_order_code && (
                    <Typography variant="body2" sx={{ mb: 1 }}>
                      注文番号 : {data.purchase_order.purchase_order_code}
                    </Typography>
                  )}
                  <Typography variant="body2">
                    発行日 : {formatIssueDate(data.purchase_order.issue_date)}
                  </Typography>
                      ...
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          <TextField
                            ...
                            onChange={(e) =>
                              handleRecordChange(index, 'description', e.target.value)
                            }
                          ...
                          <TextField
                            ...
                            onCompositionEnd={(e) => {
                              ...
                              handleRecordChange(
                                index,
                                'quantity',
                                toNumericString((e.target as HTMLInputElement).value)
                              )
                            }}
                        ...
                        <TableCell>
                          <Autocomplete
                            ...
                            onInputChange={(_, newValue, reason) => {
                              ...
                              handleRecordChange(index, 'unit', newValue)
                            }}
                            disabled={readOnly}
                            renderInput={(params) => (
                              <TextField
                        ...
                        <TableCell align="center">
                          <Select
                            ...
                            value={item.tax_rate}
                            onChange={(e) =>
                              handleRecordChange(index, 'tax_rate', Number(e.target.value))
                            }
                            size="small"
                            sx={{ fontSize: '0.8rem' }}
                            disabled={readOnly}
            ...
            <Box sx={{ mt: 4 }}>
              ...
              <TextField
                ...
                rows={3}
                variant="outlined"
                value={data.purchase_order.remarks}
