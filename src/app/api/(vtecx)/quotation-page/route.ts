...
import { URI_QUOTATION, URI_GROUP_ADMIN } from '@/utils/apiconst'
import * as util from 'utils/commonutil'
import * as apiutil from 'utils/apiutil'

export const GET = async (req: NextRequest): Promise<Response> => {
  console.log(`[api quotation-page] start.`)
  try {
    ...
    const customer_name: string = util.toString(vtecxnext.getParameter('quotation.customer_name'))
    const status: string = util.toString(vtecxnext.getParameter('quotation.status'))
    const sortParam: string = util.toString(vtecxnext.getParameter('s'))
    const isCount: boolean = vtecxnext.hasParameter('c')
    console.log(
      `[api quotation-page] n=${numStr} l=${limitStr} c=${String(isCount)} customer_name=${customer_name} status=${status} s=${sortParam}`
    )
    ...
    if (!targetCompanyCode) {
      console.log(`[api quotation-page] company_code not found. uid=${uid}`)
      if (isCount) return vtecxnext.response(200, { feed: { title: '0' } })
      return vtecxnext.response(200, { feed: {} })
    }

    // ソート条件変換
    const INDEX_SORT_FIELDS = ['issue_date', 'expiry_date']
    ...
    if (sortParam) {
      ...
      const fieldName = field.replace(/^quotation\./, '')
      if (INDEX_SORT_FIELDS.includes(fieldName)) {
        sortCondition = order === 'desc'
          ? `s=quotation.${fieldName}_desc&`
          : `s=quotation.${fieldName}&`
      } else if (DIRECT_SORT_FIELDS.includes(fieldName)) {
        sortCondition = `s=quotation.${fieldName}-${order}&`
      }
    }

    const customerNameCondition = customer_name
      ? `quotation.customer_name-ft-${encodeURIComponent(customer_name)}&`
      : ''
    const statusCondition = status
      ? `quotation.status-eq-${encodeURIComponent(status)}&`
      : ''
    const condition = customerNameCondition + statusCondition + sortCondition
    const basePath = `${URI_QUOTATION}/${targetCompanyCode}`
