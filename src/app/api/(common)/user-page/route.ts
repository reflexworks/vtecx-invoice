...
import { URI_USER } from '@/utils/apiconst'
...
export const GET = async (req: NextRequest): Promise<Response> => {
  console.log(`[api user-page] start.`)
  try {
    ...
    console.log(
      `[api user-page] n=${numStr} l=${limitStr} c=${String(isCount)} customer_name=${customer_name} status=${status} s=${sortParam}`
    )
    ...
    console.log('[api user-page] end.')
    return vtecxnext.response(resStatus, resJson)
  } catch (e) {
    return apiutil.responseError(e, 'api user-page')
  }
}
...
const count = async (vtecxnext: VtecxNext, uri: string): Promise<Response> => {
  console.log('[api user count] start.')
  console.log('[api user count] count start')
  console.log('uri:', uri)
  const result = await vtecxnext.count(uri)
  console.log('[api user count] count end')
  const resStatus = 200
  const resJson = { feed: { title: String(result) } }
  console.log('[api user count] end.')
  return vtecxnext.response(resStatus, resJson)
}
